...
if ($signingKey -and $signingKey -ne "signingkeyfilepathexample") {
    pac plugin init --signing-key-file-path $signingKey --outputDirectory $outputDir --author $author
} else {
    pac plugin init --outputDirectory $outputDir --author $author
}
cd $outputDir
...
$node = $csproj.Project.PropertyGroup.AssemblyName | Where-Object { $_ } | Select-Object -First 1
if ($node) { $node.ParentNode.RemoveChild($node) | Out-Null }
$node = $csproj.Project.PropertyGroup.PackageId | Where-Object { $_ } | Select-Object -First 1
if ($node) { $node.ParentNode.RemoveChild($node) | Out-Null }
$node = $csproj.Project.PropertyGroup.Company | Where-Object { $_ } | Select-Object -First 1
if ($node) { $node.ParentNode.RemoveChild($node) | Out-Null }
