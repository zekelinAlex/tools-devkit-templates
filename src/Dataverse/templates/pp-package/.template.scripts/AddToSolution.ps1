...
try {
    # Locate the generated csproj in the current output directory.
    $csproj = Get-ChildItem -Path . -Filter '*.csproj' -File -ErrorAction SilentlyContinue |
              Select-Object -First 1
    if (-not $csproj) { exit 0 }

    # Walk up to find the nearest .slnx (preferred) or .sln, capped at 10 levels.
    $slnFile  = $null
    ...
    # `dotnet sln add` understands both .sln and .slnx.
    & dotnet sln $slnFile.FullName add $csproj.FullName 2>&1 | Out-Null
    ...
    # Swallow everything — best-effort only.
}
