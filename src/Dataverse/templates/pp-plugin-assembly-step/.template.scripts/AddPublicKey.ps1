...
$assemblyXml = Get-ChildItem -Path "__solution-declarations-root__/PluginAssemblies" -Filter "${assemblyName}.dll.data.xml" -Recurse -ErrorAction SilentlyContinue | Select-Object -First 1

if ($null -eq $assemblyXml) {
    Write-Error "Plugin assembly XML not found for '${assemblyName}' under __solution-declarations-root__/PluginAssemblies/. Ensure the solution has been built or pp-plugin-assembly has been run."
    exit 1
}
